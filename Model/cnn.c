...
// variable and stuff
	...
	int input_mat_bs = sizeof(float)*28*28;
	memset(input_mat, 0, input_mat_bs);
	
	// layer 1 stuff - l1~~~
	const int l1_ch = 5;
	const int l1_dim = 26;
	float l1_mat[l1_ch][l1_dim][l1_dim];	// mat to store calculated value
	int l1_mat_bs = sizeof(float)*l1_ch*l1_dim*l1_dim;
	memset(l1_mat, 0, l1_mat_bs);
	Filter conv1_filter[l1_ch];

	conv1_filter[0] = (Filter){
	    .weights = {
	...
	const int l2_ch = 10;
	const int l2_dim = 24;
	float l2_mat[l2_ch][l2_dim][l2_dim];	// mat to store calculated value
	int l2_mat_bs = sizeof(float)*l2_ch*l2_dim*l2_dim;
	memset(l2_mat, 0, l2_mat_bs);
	...
	conv2_filter[0].weights[0] = (Filter[].weights[]){
        { 6.9994e-03, -1.0749e-01,  1.1241e-01 },
        { -1.2853e-01, -8.2408e-02,  9.9403e-02 },
        { 1.0726e-01, -8.2765e-02, -1.1805e-01 }
    };

	// Assuming conv2_filter is already declared as Filter conv2_filter[l2_ch];

	// // Assign weights for conv2_filter[0]
	// conv2_filter[0].weights[0][0][0] = 6.9994e-03;
	// conv2_filter[0].weights[0][0][1] = -1.0749e-01;
	// conv2_filter[0].weights[0][0][2] = 1.1241e-01;

	// conv2_filter[0].weights[0][1][0] = -1.2853e-01;
	// conv2_filter[0].weights[0][1][1] = -8.2408e-02;
	// conv2_filter[0].weights[0][1][2] = 9.9403e-02;

	// conv2_filter[0].weights[0][2][0] = 1.0726e-01;
	// conv2_filter[0].weights[0][2][1] = -8.2765e-02;
	// conv2_filter[0].weights[0][2][2] = -1.1805e-01;

	// // Assign weights for weights[1]
	// conv2_filter[0].weights[1][0][0] = -1.3585e-01;
	// conv2_filter[0].weights[1][0][1] = 5.3992e-03;
	// conv2_filter[0].weights[1][0][2] = 2.2752e-02;

	// conv2_filter[0].weights[1][1][0] = -7.2987e-03;
	// conv2_filter[0].weights[1][1][1] = 1.0242e-01;
	// conv2_filter[0].weights[1][1][2] = 1.3990e-01;

	// conv2_filter[0].weights[1][2][0] = 1.0460e-01;
	// conv2_filter[0].weights[1][2][1] = -6.6065e-02;
	// conv2_filter[0].weights[1][2][2] = 7.3180e-02;

	// // Assign weights for weights[2]
	// conv2_filter[0].weights[2][0][0] = 1.4108e-01;
	// conv2_filter[0].weights[2][0][1] = -1.2346e-01;
	// conv2_filter[0].weights[2][0][2] = 4.6408e-02;

	// conv2_filter[0].weights[2][1][0] = 1.5450e-02;
	// conv2_filter[0].weights[2][1][1] = -7.5476e-02;
	// conv2_filter[0].weights[2][1][2] = -1.2136e-01;

	// conv2_filter[0].weights[2][2][0] = 1.5864e-02;
	// conv2_filter[0].weights[2][2][1] = -2.3009e-02;
	// conv2_filter[0].weights[2][2][2] = 1.0366e-01;

	// // Assign weights for weights[3]
	// conv2_filter[0].weights[3][0][0] = 5.8840e-02;
	// conv2_filter[0].weights[3][0][1] = 1.1917e-01;
	// conv2_filter[0].weights[3][0][2] = 3.1105e-02;

	// conv2_filter[0].weights[3][1][0] = -1.2554e-01;
	// conv2_filter[0].weights[3][1][1] = 9.4659e-02;
	// conv2_filter[0].weights[3][1][2] = -4.5635e-02;

	// conv2_filter[0].weights[3][2][0] = 1.3812e-02;
	// conv2_filter[0].weights[3][2][1] = 1.3762e-02;
	// conv2_filter[0].weights[3][2][2] = 1.0526e-01;

	// // Assign weights for weights[4]
	// conv2_filter[0].weights[4][0][0] = -4.8520e-02;
	// conv2_filter[0].weights[4][0][1] = -1.3673e-01;
	// conv2_filter[0].weights[4][0][2] = 9.9640e-02;

	// conv2_filter[0].weights[4][1][0] = -4.5110e-02;
	// conv2_filter[0].weights[4][1][1] = -5.6077e-02;
	// conv2_filter[0].weights[4][1][2] = 7.9168e-02;

	// conv2_filter[0].weights[4][2][0] = -1.0438e-02;
	// conv2_filter[0].weights[4][2][1] = -6.5131e-02;
	// conv2_filter[0].weights[4][2][2] = 3.4479e-02;

	// // Assign bias
	// conv2_filter[0].bias = 0.0311;
...
// Calculation
	...
	conv2d(input_mat, l1_mat, conv1_filter, 28, NUM_01CH, NUM_05CH);
	relu(l1_mat, 26, NUM_05CH);
	...
	printMatrix(l1_mat, NUM_10CH, 26);
			...
			for (int n = 0; n < AFTER_KERNEL(in_dim); n++)	// n for col
			{
				...
				for (int i_in_ch = 0; i_in_ch < in_ch; i_in_ch++)	// iterate though input channels
				{
					...
					for (int p = 0; p < FILTER_SIZE; p++)			// for accessing row of filter
					{
						...
						{
							sum += in_mat[i_in_ch][m + p][n + q] * filter[i_out_ch].weights[i_in_ch][p][q];
						}
					}
					// END: iterate though Kernel
				}
				out_mat[i_out_ch][m][n] += sum + filter[i_out_ch].bias;
			}
		}
